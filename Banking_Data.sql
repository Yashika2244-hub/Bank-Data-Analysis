...
#Transactions done by employees
 ...
 select b.branch_location,e.employee_id,e.first_name,sum(t.amount),count(t.transaction_id)from accounts a
  join branches b on a.branch_id = b.branch_id
  join employees e on e.branch_id = a.branch_id
  join transactions t on t.account_number = a.account_number
  group by e.employee_id,b.branch_location,e.first_name
